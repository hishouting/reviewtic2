...
//*******************************************************/
						...
						assure(infile, filename);
		                string strrr;//N是定义的常数，目的是为了读取足够长的行
		 				// int n=0;//用来表示说读/写文本的行数
		 				// cout << "hiihii\n";
						while(getline(infile,strrr))
						{
						 // infile.getline(strrr, sizeof(strrr));//此处默认的终止标识符为‘\n’
						 if(strrr != "\n")
						 	ccc++;
						}
						// cout<<n;
			...
			if ((fileinfo.attrib & _A_SUBDIR))
			{
					...
					(fileinfo.name,"..") != 0)
				{
					...
					for (vector<string>::iterator it=tmp.begin();it<tmp.end();it++)
					{
						...
						// //*******************************************************/
						// ifstream infile(fileinfo.name);//此处默认的文件打开方式为“以输出的方式打开”
						// assure(infile, fileinfo.name);
		    //             string strrr;//N是定义的常数，目的是为了读取足够长的行
		 			// 	// int n=0;//用来表示说读/写文本的行数
		 			// 	cout << "hiihii\n";
						// while(getline(infile,strrr))
